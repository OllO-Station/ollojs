...
export interface NftMarketSDKType {
  id: string;
}
...
function createBaseNftMarket(): NftMarket {
  return {
    id: ""
  };
}
...
export const NftMarket = {
  encode(message: NftMarket, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.id !== "") {
      writer.uint32(10).string(message.id);
    }
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): NftMarket {
    const reader = input instanceof _m0.Reader ? input : new _m0.Reader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseNftMarket();
  ...
  fromPartial(object: DeepPartial<NftMarket>): NftMarket {
    const message = createBaseNftMarket();
    message.id = object.id ?? "";
    return message;
  }
