...
export class QueryClientImpl implements Query {
  ...
  params(request: QueryParamsRequest = {}): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("ollo.loan.v1.Query", "Params", data);
    return promise.then(data => QueryParamsResponse.decode(new _m0.Reader(data)));
  }

  loans(request: QueryGetLoansRequest): Promise<QueryGetLoansResponse> {
    const data = QueryGetLoansRequest.encode(request).finish();
    const promise = this.rpc.request("ollo.loan.v1.Query", "Loans", data);
  ...
  loansAll(request: QueryAllLoansRequest = {
    ...
    const promise = this.rpc.request("ollo.loan.v1.Query", "LoansAll", data);
